...
/*   Updated: 2017/06/27 02:32:53 by cchaumar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "printf.h"

static int	set(void *p, int v, size_t size, int ret)
{
	int		i;
...
static int	conv_numbers(char *s, t_field *f)
{
	if (*s == '.')
	{
		if (s[1] == '*')
...
int			conv(char *s, t_field *f)
{
	...
	return (conv_numbers(s, f));
}

inline void	p_reduce(t_field *f)
{
